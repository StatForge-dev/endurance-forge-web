...
export function experimentalMetrics(analysis){
 ...
 // Piecewise breakpoint: smooth efficiency first, search central 20–80%, and require a meaningful late deterioration.
 const efSmooth=movingMean(x,'ef',3), xf=x.map((r,i)=>({...r,ef:efSmooth[i]})).filter(r=>finite(r.ef));
 let best=null; const base=linfit(xf); for(let f=.20;f<=.80;f+=.025){const t=start+f*dur,a=xf.filter(r=>r.t<=t),b=xf.filter(r=>r.t>t);if(a.length<18||b.length<18)continue;const fa=linfit(a),fb=linfit(b);if(!fa||!fb)continue;const sse=fa.sse+fb.sse;if(!best||sse<best.sse)best={t,f,sse,pre:fa.slope,post:fb.slope}}
 let fip=null;if(best&&base&&base.sse>0){const improve=1-best.sse/base.sse,efMean=mean(xf.map(r=>r.ef))||1,postPctHr=best.post/efMean*3600*100,prePctHr=best.pre/efMean*3600*100;const deterioration=postPctHr-prePctHr;if(improve>.035&&postPctHr<-.35&&deterioration<-.35)fip={timeS:best.t-start,pct:best.f*100,declinePctHr:postPctHr,improvement:improve,confidence:confidence(xf.length,improve>.08&&postPctHr<-1)}}
 ...
 const rec=[],hrSmooth=movingMean(x,'hr',2);let lastPeak=-999;for(let i=4;i<x.length-14;i++){if(i-lastPeak<12)continue;const before=hrSmooth.slice(Math.max(0,i-4),i),after=hrSmooth.slice(i+1,Math.min(x.length,i+25));if(!before.length||after.length<12)continue;const peak=hrSmooth[i],preMax=Math.max(...before),futureMin=Math.min(...after),drop=peak-futureMin;if(peak<preMax-1||drop<6)continue;const h60=interp(x,x[i].t+60,'hr'),hrr60=finite(h60)?peak-h60:NaN;if(!finite(hrr60)||hrr60<3)continue;const floor=median(after.slice(Math.max(0,after.length-8)));if(!finite(floor)||peak-floor<5)continue;const ys=[];const endi=Math.min(x.length-1,i+24);for(let j=i+1;j<=endi;j++){const hv=hrSmooth[j],ratio=(hv-floor)/(peak-floor);if(ratio>.03&&ratio<.98)ys.push({t:x[j].t-x[i].t,ef:Math.log(ratio)})}const fit=linfit(ys),tau=fit&&fit.slope<0?-1/fit.slope:NaN;if(finite(tau)&&tau>=5&&tau<=360&&ys.length>=5){const curve=x.slice(i,endi+1).map((r,k)=>({t:r.t-x[i].t,hr:hrSmooth[i+k],relative:hrSmooth[i+k]-peak}));rec.push({t:x[i].t-start,hrr60,tau,peakHr:peak,floor,curve});lastPeak=i}}
